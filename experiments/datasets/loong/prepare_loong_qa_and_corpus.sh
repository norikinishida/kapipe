...
# Results:
#   - STORAGE_DATA/loong/corpus/{financial,legal,paper}_{en,zh}.articles.jsonl
#   - STORAGE_DATA/loong/qa/{financial,legal,paper}_{en,zh}.json
#   - STORAGE_DATA/loong/qa/{financial,legal,paper}_{en,zh}.gold_contexts.json
python prepare_loong_qa_and_corpus.py \
    --input_questions_file "${LOONG_REPOSITORY}/data/loong.jsonl" \
    --input_documents_dir "${LOONG_REPOSITORY}/data/doc" \
    --output_articles_dir "${STORAGE_DATA}/loong/corpus" \
    --output_qa_dir "${STORAGE_DATA}/loong/qa"
