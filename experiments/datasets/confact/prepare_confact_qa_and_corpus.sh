...
# Results:
#   - STORAGE_DATA/confact/corpus/articles.jsonl
#   - STORAGE_DATA/confact/qa/{modc,humc}.json
#   - STORAGE_DATA/confact/qa/{modc,humc}.gold_contexts.json
python prepare_confact_qa_and_corpus.py \
    --input_modc_file "${CONFACT}/${MODC_FILENAME}" \
    --input_humc_file "${CONFACT}/${HUMC_FILENAME}" \
    --output_articles_file "${STORAGE_DATA}/confact/corpus/articles.jsonl" \
    --output_dir "${STORAGE_DATA}/confact/qa"
