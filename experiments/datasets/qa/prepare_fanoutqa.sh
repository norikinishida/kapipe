...
OUTPUT_ARTICLES_FILE="${STORAGE_DATA}/articles/fanoutqa/${WIKIPEDIA_DUMP_BASE}.articles.jsonl"
OUTPUT_QA_DIR="${STORAGE_DATA}/qa/fanoutqa"
...
# Results:
#   - STORAGE_DATA/articles/fanoutqa/WIKIPEDIA_DUMP_BASE.articles.jsonl
if [ ! -f "${STORAGE_DATA}/articles/fanoutqa/${WIKIPEDIA_DUMP_BASE}.articles.jsonl" ]; then
    python ../articles/prepare_wikipedia_articles.py \
        --input_dir "${FANOUTQA}/wikipedia/${WIKIPEDIA_DUMP_FILENAME}.extracted" \
        --output_file "${OUTPUT_ARTICLES_FILE}"
fi

# Results:
#   - STORAGE_DATA/qa/fanoutqa/{dev,test}.json
#   - STORAGE_DATA/qa/fanoutqa/{dev,test}.gold_contexts.json
python prepare_fanoutqa.py \
