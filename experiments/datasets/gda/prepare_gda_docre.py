...
def main(args):
    assert args.input_file.endswith(".data")
    ...
    with open(path_input_file, "r") as f:
        for line in f:
            ...
            doc_key = f"gda/{args.split}/{line[0]}"
            text = line[1]
            ...
            n_gene_mentions += sum([len(e["mention_indices"]) for e in record["entities"] if e["entity_type"] == "Gene"])
            n_disease_mentions += sum([len(e["mention_indices"]) for e in record["entities"] if e["entity_type"] == "Disease"])
            n_gene_entities += len([e for e in record["entities"] if e["entity_type"] == "Gene"])
    ...
    print(f"Average number of gene mentions per doc: {n_gene_mentions}/{len(doc_key_list)} = {float(n_gene_mentions) / len(doc_key_list)}")
    ...
    print(f"Average number of gene entities per doc: {n_gene_entities}/{len(doc_key_list)} = {float(n_gene_entities) / len(doc_key_list)}")
...
def get_chunks(lst):
    ...
    for i in range(0, len(lst), chunk_length):
        ...
        dct = {
            "relation": chunk[0], # 1:GDA:2, 1:NR:2, not_include
        ...
        assert dct["relation"] in ["1:GDA:2", "1:NR:2", "not_include"], dct["relation"]
        if dct["relation"] in ["1:GDA:2", "1:NR:2"]:
            dct["relation"] = dct["relation"].split(":")[1]
        assert dct["arg1_entity_type"] == "Gene"
...
def get_relations(chunks, entity_id_to_index):
    relations = []
    for chunk in chunks:
        if chunk["relation"] == "GDA":
            assert chunk["arg1_entity_type"] == "Gene"
