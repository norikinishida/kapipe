...
#   - STORAGE_DATA/streamingqa/corpus/articles.jsonl
#   - STORAGE_DATA/streamingqa/qa/{train,dev,test}.json
#   - STORAGE_DATA/streamingqa/qa/{train,dev,test}.gold_contexts.json
python prepare_streamingqa_qa_and_corpus.py \
    --input_dir "${STREAMINGQA}" \
    --wmt_dir "${WMT_NEWS_CRAWL}" \
    --extraction_file "${STREAMINGQA}/google-deepmind.streamingqa/extraction.py" \
    --output_articles_file "${STORAGE_DATA}/streamingqa/corpus/articles.jsonl" \
    --output_questions_dir "${STORAGE_DATA}/streamingqa/qa"

# Results:
#   - STORAGE_DATA/streamingqa/qa/{train,dev,test}_filtered.json
python filter_streamingqa.py \
    --input_questions_dir "${STORAGE_DATA}/streamingqa/qa" \
    --output_questions_dir "${STORAGE_DATA}/streamingqa/qa"

# Results:
#   - STORAGE_DATA/streamingqa/corpus/articles_for_test_filtered.jsonl
python reduce_articles_for_streamingqa.py \
    --input_questions_file "${STORAGE_DATA}/streamingqa/qa/test_filtered.json" \
    --input_gold_contexts_file "${STORAGE_DATA}/streamingqa/qa/test.gold_contexts.json" \
    --input_articles_file "${STORAGE_DATA}/streamingqa/corpus/articles.jsonl" \
    --index_dir "${STORAGE_DATA}/streamingqa/corpus/contriever_index" \
    --output_articles_file "${STORAGE_DATA}/streamingqa/corpus/articles_for_test_filtered.jsonl"
